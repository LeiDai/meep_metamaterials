...
class material_dielectric():#{{{
    ...
    The predefined permittivity 2. holds e. g. for polyethylene, teflon etc. and other materials that 
    have a flat index of refraction ~ 1.4 both at microwave and optical frequencies
    """
    def __init__(self, where=None, eps=2., loss=0):
        self.eps = eps*(1-loss)
        if loss != 0:
            self.pol = [
...
class material_DrudeMetal():#{{{
    ...
    def __init__(self, where=None, lfconductivity=15e-6, f_c=1e14):
        """
        ...
        self.pol = [
                {'omega': f_0, 'gamma': self.gamma, 'sigma': f_p**2 / f_0**2}, # (Lorentz) model for Al
                ]
